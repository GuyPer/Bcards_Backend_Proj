...
const mustLogin = (req,res,next) => {
  ...
  try {
    ...
    return res.status(403).json({ sucees: false, message: 'Forbidden: invalid token' })
  }
}
...
const allowedRoles = (allowedRoles) => {
  return async (req,res,next) => {
    ...
    const { isBusiness, isAdmin, id } = req.user;
    ...
    if (allowedRoles.includes('registedUser') && id===req.params.id) hasRole=true;
    if (allowedRoles.includes('loggedIn') ) hasRole=true;
    if (allowedRoles.includes('isLiked') ) hasRole=true;
    if (allowedRoles.includes('isBusiness') ) hasRole=true;
    if (allowedRoles.includes('owner')) {
      try {
        const card = await Card.findById(req.params.id);
        if (card && card.user_id.toString() === id) hasRole = true;
      } catch (err) {
        return res.status(500).json({ success: false, message: `Error checking card owner: ${err.message}` });
      }
    }
